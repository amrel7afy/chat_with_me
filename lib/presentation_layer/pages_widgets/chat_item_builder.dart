...
import 'package:chat_app/business_logic_layer/chat_cubit/chat_states.dart';
import 'package:chat_app/business_logic_layer/listen_to_messages_cubit/listen_to_messages_cubit.dart';
import 'package:chat_app/business_logic_layer/listen_to_messages_cubit/listen_to_messages_state.dart';
...
class ChatItemBuilder extends StatefulWidget {
  ...
  const ChatItemBuilder({Key? key, required this.chatModel}) : super(key: key);
...
class _ChatItemBuilderState extends State<ChatItemBuilder> {
  ...
  void initState() {
    ListenToMessagesCubit.getCubit(context).listenToMessages(
        recieverId: '+2${widget.chatModel.phoneNumber}', context: context);
